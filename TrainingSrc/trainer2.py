...
def load_and_prepare_data(train_path, test_path):
    ...
    test_texts = test_df['cuerpo'].tolist()
    test_labels = test_df['authorship'].tolist()
    test_encoded_labels = label_encoder.fit_transform(test_labels)
    
    train_df = pd.read_csv(train_path)
    train_texts = train_df['cuerpo'].tolist()
    train_labels = train_df['authorship'].tolist()
    train_encoded_labels = label_encoder.fit_transform(train_labels)
    
    return train_texts, test_texts, train_encoded_labels, test_encoded_labels, label_encoder
...
def train_model(test_path, train_path, model_name="dccuchile/bert-base-spanish-wwm-uncased", output_dir="./results"):
    device = check_gpu_availability()
    
    print("Loading and preparing data...")
    train_texts, val_texts, train_labels, val_labels, label_encoder = load_and_prepare_data(test_path=test_path, train_path=train_path)
...
def predict_with_trained_model(text, model_path, tokenizer_path=None):
    ...
    return {
        ...
        'all_probabilities': {
            label: prob.item() 
            for label, prob in zip(label_encoder.classes_, predictions[0].cpu())  # Move to CPU for numpy conversion
        }
    }
...
if __name__ == "__main__":
    ...
    trainer, label_encoder = train_model(
        ...
        test_path=test_path,
        model_name="dccuchile/bert-base-spanish-wwm-uncased", # base model used for training
        output_dir=folderPath + "/BETenio"
    )
